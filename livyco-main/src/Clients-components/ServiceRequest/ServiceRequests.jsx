      ...
      {serviceRequests.map((request) => (
        <div key={request.id} className="bg-white p-6 rounded-lg border border-[#BCBCBC] mb-4">
          ...
          <div className="flex items-center space-x-2 mb-2">
            <span className="bg-[#FFDC82] px-3 py-1 text-sm rounded">{request.roomDetails}</span>
            <span className="bg-[#FFDC82] px-3 py-1 text-sm rounded">{request.sharingType}</span>
          </div>
          ...
          <div className="flex space-x-4">
            <button
              className="px-4 py-2 bg-[#0827B2] text-white rounded-3xl hover:bg-blue-700"
              onClick={() => navigate(`/client/servicerequests/approval/${request.id}`, { state: request })}
            >
              Approve Request
            </button>
            <button
                className="px-4 py-2 border border-[#0827B2] text-blue-600 rounded-3xl hover:bg-blue-100"
